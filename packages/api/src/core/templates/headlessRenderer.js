...
/**
 * Headless Slide Renderer for high-resolution post assets
 */
export async function renderTemplateSlide(templateSchema, slotData, brandVariables, slideIndex = 0, customDimensions = null) {
  const p = await getPageInstance();
  
  const width = customDimensions?.width || templateSchema?.dimensions?.width || 1080;
  const height = customDimensions?.height || templateSchema?.dimensions?.height || 1080;

  // Set the viewport matching computed target dimensions
  await p.setViewport({ width, height });

  // Evaluate drawing commands inside the pre-warmed Puppeteer context
  await p.evaluate(async (schema, data, brandVars, w, h, sIdx) => {
    const canvasEl = document.getElementById('canvas');
    canvasEl.width = w;
    canvasEl.height = h;
    ...
    const activeSlide = schema.slides?.[sIdx];
    if (!activeSlide) return;

    const components = activeSlide.components || [];
    const slotId = activeSlide.slot_id || `slide_${sIdx + 1}`;
    const currentSlot = data[slotId] || {};
    const palette = currentSlot.palette || {};
    ...
    fCanvas.renderAll();
  }, templateSchema, slotData, brandVariables, width, height, slideIndex);
